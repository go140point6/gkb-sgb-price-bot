...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const gkb_ADDRESS = "0x1915462fece97854d6738b90d3f9ec09e723d451"
  const gkb = "gkb"
  ...
  let gkbSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let gkbUSDPrice = (sgbPrice / gkbSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(gkb + " USD price is " + gkbUSDPrice)

  clearRoles()
  lastPrice = gkbUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${gkbSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${gkbUSDPrice}`)
